...
from app.core.runtime import QThread, Signal
...
def is_valid_auto_paused(info: Any) -> bool:
    """校验 config.scheduler.auto_paused 持久化标志 schema。

    合法：dict 且含 reason(非空 str)/fail_count(int)/ts(正数)。
    非法值视为 None 放行（由调用方写 WARNING 并清掉脏值）。
    """
    ...
    try:
        ...
        return False
    return bool(info.get("reason")) and ts > 0 and fail_count >= 0
...
class Scheduler(QThread):
    ...
    def __init__(
        ...
        # 第二级硬保护：连续失败真正暂停（持久化在 config.scheduler.auto_paused）
        # None=未暂停；dict={reason, fail_count, last_kind, ts}
        self._auto_paused_info: dict | None = None
    ...
    # ---------------- 第二级失败暂停（人工恢复是唯一出口）----------------
    def apply_persisted_pause(self, info: Any) -> bool:
        """启动后恢复持久化的失败暂停态。schema 非法返回 False（视为无暂停）。"""
        if not is_valid_auto_paused(info):
            return False
        fail_count = int(info.get("fail_count") or 0)
        ts = float(info.get("ts") or 0)
        data = {
            ...
            "last_kind": str(info.get("last_kind") or "unknown"),
            "ts": ts,
        }
        with self._state_lock:
            self._auto_paused_info = data
            self._fail_streak = fail_count
        self.pause()
        self._set_state(f"失败自动暂停（连续 {fail_count} 次，需手动恢复）")
        log.warning("恢复持久化的失败暂停标志：%s", data)
        return True

    def manual_resume(self) -> None:
        """人工恢复（自动暂停的唯一出口）：清持久标志 + 复位计数/冷却档 + 放行。

        - 清除 config.scheduler.auto_paused（重启也不再保持暂停）
        - _fail_streak / _empty_read_streak / _risk_cooldown_level /
          _risk_pause_until 全部复位
        - 跨天不自动恢复：只有本方法被显式调用才恢复
        """
        self._cfg.set("scheduler.auto_paused", None)
        with self._state_lock:
            self._auto_paused_info = None
            self._fail_streak = 0
            self._empty_read_streak = 0
            self._risk_cooldown_level = 0
        self._risk_pause_until = 0.0
        self._emit_log("▶️ 已手动恢复：失败计数与冷却档位已复位")
        self.resume()

    def regenerate_plan(self) -> None:
        """清空今日目标（下一轮 _ensure_daily_plan 重新随机；调试用）。"""
        self._cfg.set("daily_plan", {"date": "", "target_minutes": 0})
        self._emit_log("🔄 已清空今日目标，下一轮将重新随机生成")

    def get_status(self) -> dict[str, Any]:
        """5911 面板 /api/scheduler/status 用的完整状态快照。"""
        with self._state_lock:
            state = self._current_state
            book = dict(self._current_book) if self._current_book else None
            chapter = dict(self._current_chapter) if self._current_chapter else None
            chapter_read_count = self._chapter_read_count
            success = self._success_count
            fail = self._fail_count
            paused = not self._pause_event.is_set()
            auto_paused = (
                dict(self._auto_paused_info) if self._auto_paused_info else None
            )
            next_run_at = self._next_run_at

        today_seconds = self._db.get_today_seconds()
        stats = self._db.get_reading_stats()
        target = int(self._cfg.get("daily_plan.target_minutes", 0) or 0)
        now = time.time()
        progress_pct = (
            round(today_seconds / 60.0 / target * 100, 1) if target > 0 else 0.0
        )
        book_info = None
        if book:
            book_info = {
        ...
        return {
            ...
            "daily_plan_date": self._cfg.get("daily_plan.date", ""),
            ...
            "fail_count": fail,
            "next_run_at": round(next_run_at, 3) if next_run_at else None,
            "next_run_in_sec": max(0, int(next_run_at - now)) if next_run_at else None,
            "book": book_info,
            "chapter": chapter_info,
            "stats": {
                "today_seconds": stats.get("today_seconds", 0),
                "week_seconds": stats.get("weekly_seconds", 0),
                "month_seconds": stats.get("monthly_seconds", 0),
    ...
    def run(self) -> None:  # noqa: D401
        ...
        while not self._stop_event.is_set():
            ...
            if not self._pause_event.is_set():
                # 第二级失败暂停与手动暂停区分状态文案；
                # 自动暂停在此无限等待，跨天也不自动恢复（仅 manual_resume 放行）
                if self._auto_paused_info:
                    self._set_state(
                        f"失败自动暂停（连续 "
                        f"{self._auto_paused_info.get('fail_count')} 次，需手动恢复）"
                    )
                else:
                    self._set_state("已暂停")
                # wait() 会阻塞到被外部 set()（即 resume()/manual_resume()）
                self._pause_event.wait()
                if self._stop_event.is_set():
                    return
                if not self._auto_paused_info:
                    self._set_state("运行中")
    ...
    def _do_one_reading_step(self) -> bool:
        ...
        if not ok and kind != "session_invalid":
            ...
            if risk_event:
                ...
                cooldown_armed = True
            # 第二级硬保护：连续失败达 fail_pause_threshold → 真正暂停
            # （第一级的指数冷却仍保留；0=禁用第二级）。Cookie 硬失效不计入。
            self._maybe_enter_failure_pause(fail_streak, kind)
        return ok

    def _maybe_enter_failure_pause(self, fail_streak: int, last_kind: str) -> bool:
        """达到第二级阈值则真正暂停并持久化，返回是否触发。"""
        try:
            threshold = int(self._cfg.get("risk.fail_pause_threshold", 15))
        except (TypeError, ValueError):
            threshold = 15
        if threshold <= 0 or fail_streak < threshold:
            return False
        info = {
            "reason": "read_fail",
        ...
        with self._state_lock:
            self._auto_paused_info = info
        # 先持久化再暂停：确保重启不会绕过保护
        self._cfg.set("scheduler.auto_paused", info)
        self._emit_log(
            f"⛔ read 接口连续失败 {fail_streak} 次（最近类型：{last_kind}），"
            "触发第二级保护：调度器真正暂停，不自动恢复、跨天不恢复；"
            "请在 5911 面板查看日志排查后点「手动恢复」"
        )
        try:
            self._notifier.notify_auto_paused(fail_streak, last_kind)
        except Exception as exc:  # noqa: BLE001
            log.warning("自动暂停通知发送异常：%s", exc)
        self.pause()
        self._set_state(f"失败自动暂停（连续 {fail_streak} 次，需手动恢复）")
        return True
