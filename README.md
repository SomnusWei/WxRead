...
| **混合完成度检测** | 本地实时累加阅读秒数 + 每 30 分钟 Skill 官方数据权威校准双保险，精准判断当日达标 |
| **阅读统计 4 项** | 通过 `/readdata/detail` 获取今日 / 本周 / 本月 / 总累计 4 项权威时长；今日时长以官方数据为基线定期校准，本地估算仅在基线之上实时累加 |
| **套装书自动跳过** | 遇到 CB_/MP_ 等非数字 bookId 的套装书、公众号合集（无法自动上报）时立即永久拉黑并自动换书；黑名单持久化，重启/书架重同步不丢失 |
| **阅读风控自动防线** | read 上报结果 8 类精细分类；连续 3 次空响应（软风控特征）或 8 次连续失败即自动停止一切上报，指数退避冷却（60→120→…→360 分钟），冷却结束自动试探、成功自动恢复 |
| **可视化报告** | 聚合 LocalDB + Skill 数据，弹出报告页：读者画像 / 书架归一化饼图（进度+可见性双维度）/ 阅读趋势线 / 月度热力图 / 20 条阅读高光，纯 QPainter 绘制无额外依赖 |
| **风控规避** | 随机书籍/章节切换、±15% 间隔抖动、每 N 次成功插入长休息、动态 `baggage`/`sentry-trace` 请求头；软风控时自动冷却并 WxPusher 告警（同类 3 小时限频，持久化不重复轰炸） |
| **WxPusher 推送** | 每日任务开始/完成、Cookie 失效、阅读风控告警、Skill 登录成功等事件推送（消息中时长以「X小时Y分钟」呈现） |
...
- **今日阅读时长** — 当日累计秒数（官方数据为权威基线，每 30 分钟校准一次）
- **本周累计时长** — 本周一至今累计
- **本月累计时长** — 本月累计
- **总累计时长** — 账号历史总阅读时长

> 今日时长口径：每次上报本地固定 +45s 仅为实时估算（会略偏大），Skill 刷新时若官方值 > 0 则以官方值覆盖为新基线、随后继续累加；官方暂时返回 0（接口抖动/当天无数据）时保留本地值不误清零；跨天自动重置。校准时日志会显式输出「已按微信读书官方数据校准上调/下调」。

### 🛡️ 阅读风控自动防线

- read 接口每次响应被分为 `ok / empty_200 / errcode_xxx / bad_response / http_error / network / internal_error / session_invalid` 八类，失败日志直接附带类型。
- **软风控识别**：登录态正常（chapterInfos 等接口可用）但 read 连续 3 次返回 HTTP 200 空 `{}`（服务端静默丢弃），即判定软风控。
- **自动冷却**：命中后立即停止一切上报进入冷却，60 → 120 → 240 分钟逐档翻倍（360 分钟封顶），主界面实时显示冷却倒计时；冷却结束自动试探，成功一次即恢复。
- **告警**：WxPusher 推送「🚨 阅读风控提醒」，同类告警 3 小时最多一条，限频时间戳持久化，重启也不会重复轰炸。
- 阈值均在配置文件 `risk` 节可调（判定阈值、冷却时长、告警间隔）。

### 📚 不支持书籍永久跳过

套装书 / 公众号合集的 bookId 为 `CB_`/`MP_` 等非数字格式，其"章节"实际是多本完整子书，read 接口对这类 ID 一律返回空响应。选书时识别到即自动拉黑并立即换书，黑名单保存在 `local_db.json` 顶层 `blacklist`，书架重新同步后标记仍保留。
...
调度循环：
  ...
  5. read_once 单次阅读上报（30~45s ± 15% 抖动），响应分类写入风控判定
  6. 本地累加进度与今日阅读秒数，触发 UI 更新（卡片与状态区同源实时刷新）
  7. 计算下次间隔（成功/失败/长休息不同策略）
  8. 每 30 分钟用 Skill 官方数据校准进度与统计
  9. 命中软风控（连续空响应/连续失败）→ 停止上报，指数退避冷却后自动试探
```
...
├── tools/
│   └── keygen_app.py          # 【开发者专用】注册码生成器（tkinter GUI，可 PyInstaller 打成独立 exe）
...
正式二进制见 **[GitHub Releases](https://github.com/SomnusWei/WxRead/releases)**，每个版本**仅提供一个安装包**：

**下载 `WxReadAssistant-vX.Y.Z-setup.exe`**，双击安装向导完成安装（自动创建开始菜单/桌面快捷方式，自带卸载器）。

> 大二进制不进入 git 历史（`.gitignore` 已过滤）；GitHub 仅发布 setup.exe，历史旧版本 release 会定期清理。
...
重新运行最新 `WxReadAssistant-vX.Y.Z-setup.exe` 即可：安装器使用固定 AppId，会**原地升级**到安装目录，不触碰 `%APPDATA%` 下的用户数据（Cookie / 配置 / 进度 / 日志）。升级前请先退出程序（含托盘图标 → 右键 → 退出）。

> 开发者构建仍支持免安装 full 目录与增量补丁（`build_release.py` 产物），但不再随 GitHub Release 分发。
...
python scripts\build_release.py --version 2.3.1 --previous 2.3.0

# 打包开发者专用注册码生成器（单文件 GUI exe，输出到 tools\dist\）
pyinstaller --onefile --windowed --name WxReadKeygen --paths . --icon assets\app_icon.ico --distpath tools\dist tools\keygen_app.py
```

> 增量补丁的删除清单会自动过滤 Inno 卸载器（`unins000.*`），补丁更新不影响正常卸载；构建内置杀软文件锁重试。
> `WxReadKeygen.exe` 含注册码算法密钥，**仅供开发者本地使用，禁止随 Release 公开发布**。
...
| 🛡️ 风控 | 软风控判定阈值 | 连续 3 次 HTTP 200 空响应 |
| 🛡️ 风控 | 连续失败判定阈值 | 连续 8 次失败 |
| 🛡️ 风控 | 冷却时长 | 60 分钟起指数翻倍，封顶 360 分钟 |
| 🛡️ 风控 | 风控告警限频 | 同类 180 分钟最多一条（持久化） |
...
- **混合完成度**：本地实时累加秒数（每 30~45 秒 +45s 估算）+ 每 30 分钟 Skill 官方数据权威校准（官方值 >0 覆盖为新基线、0 保留本地、跨天丢弃旧值），双保险判定当日达标。
- **风控规避**：随机书籍/章节切换、间隔抖动、长休息、动态 `baggage`/`sentry-trace` 请求头匹配浏览器指纹。
- **软风控自动冷却**：read 响应 8 分类，连续空响应/连续失败即指数退避冷却，停止全部上报，到期自动试探、成功自动恢复，WxPusher 限频告警。
- **不支持书黑名单**：`CB_`/`MP_` 类 bookId 永久拉黑，持久化在 `local_db.json`，书架重同步不复活。
...
- 修复 Skill 官方统计刷新后今日时长不更新：旧逻辑对本地估算与官方值取 max，本地每次固定 +45s 的估算系统性偏大（如本地 49 分钟 vs 官方 36 分钟），导致官方数据永远无法纠正显示。改为官方值 >0 时以官方数据为权威基线覆盖、随后本地继续实时累加；官方返回 0/接口失败时保留本地值不清零；跨天丢弃外部旧值。
- 统计卡片改为每轮上报后实时刷新，卡片与状态区时长保持一致；Skill 校准幅度 ≥1 分钟时日志输出「今日时长已按微信读书官方数据校准上调/下调 X → Y 分钟」。
- 构建：增量补丁删除清单自动过滤 Inno 卸载器（`unins000.*`），避免补丁更新后无法卸载；复制/压缩环节增加杀软文件锁重试。
...
#### 🛡️ 阅读风控自动防线
- `weread_api.read_once` 响应 8 类精细分类（ok / empty_200 / errcode / bad_response / http_error / network / internal_error / session_invalid），失败日志直接附带类型。
- scheduler 新增软风控检测：登录态正常但 read 连续 3 次返回 HTTP 200 空 `{}` 即判定软风控；连续 8 次任意失败判通用失败。
- 命中后指数退避自动冷却（60 → 120 → … → 360 分钟封顶），期间停止一切上报，主界面显示冷却倒计时；冷却结束自动试探，成功一次即恢复。
- WxPusher 新增「🚨 阅读风控提醒」推送，同类告警 3 小时限频，时间戳持久化重启不重复轰炸；阈值全部可在 `config.json` 的 `risk` 节配置。

#### 📚 套装书/公众号合集永久跳过
- read 端点对 `CB_`/`MP_` 等非数字 bookId 一律返回空 `{}`；选书识别后立即永久拉黑并自动换书。
- 黑名单持久化于 `local_db.json` 顶层 `blacklist`，书架重新同步后标记保留；`clear_all()` 不影响黑名单。
